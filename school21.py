...
model = genai.GenerativeModel("gemini-1.5-flash", system_instruction="Ты — чат-бот по имени Astro. Помогаешь людям, которые интересуются поступлением в IT-школу School 21. Отвечай чётко, кратко и по делу на основе информации о школе. Если знаешь ответ — отвечай напрямую. Если не знаешь — скажи, что не уверен, и предложи обратиться в School 21 напрямую. Если получаешь аудио, просто повтори его дословно. Целевая аудитория — люди, желающие поступить в School 21")
...
# ABOUT
  ...
  {"role": "user", "parts": "Бассейн — интенсив."},
# CONTACTS
